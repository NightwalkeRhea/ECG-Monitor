...
module ecg_top (
    ...
    /**
     *  Multi-stage reset release (async assertion, sync deassertion)
     *  Avoids metastability at power-on, ensures all modules reach stable state
     *  RST_N is directly used for async assertion (fastest response to button press)
     *  rst_release_sync creates 2-stage synchronizer for deassert path
     *  rst_n_core is the synchronized reset used by all internal modules
     */
     ...
     * - BUZZER_HALF_PERIOD = 6750 clk cycles per half-period
     * - 27 MHz / (2 × 6750) = 2000 Hz square wave frequency
     * - Generated only when alarm_condition is active (energy efficient)
     * - Drives passive buzzer through NPN switch (see schematic for flyback diode)
     */
    ...
    wire sample_recent = (sample_activity_ctr != 24'd0);
    wire qrs_recent = (qrs_activity_ctr != 24'd0);
    ...
    always @(posedge clk_core) begin
        if (!rst_n_core) begin
            ...
            if (qrs_detected) begin
                qrs_activity_ctr <= SAMPLE_ACTIVITY_HOLD;
            end else if (qrs_activity_ctr != 24'd0) begin
                qrs_activity_ctr <= qrs_activity_ctr - 24'd1;
            end
        end
    end
    ...
    assign DBG_LED2_N = ~(sample_recent & debug_blink_fast);   // valid 16-bit samples accepted
    assign DBG_LED3_N = ~(qrs_recent & debug_blink_fast);   // recent QRS detections
